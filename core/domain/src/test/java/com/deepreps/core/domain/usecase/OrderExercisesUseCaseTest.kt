...
class OrderExercisesUseCaseTest {
    ...
    fun `compounds appear before isolations`() {
        val isolation = makeExercise(
            id = 1, name = "Leg Extension", movementType = MovementType.ISOLATION,
            primaryGroupId = 1, orderPriority = 50,
        )
        val compound = makeExercise(
            id = 2, name = "Barbell Squat", movementType = MovementType.COMPOUND,
            primaryGroupId = 1, orderPriority = 10,
        )
    ...
    fun `within compounds, lower orderPriority comes first`() {
        val chestPress = makeExercise(
            id = 1, name = "Bench Press", movementType = MovementType.COMPOUND,
            primaryGroupId = 4, orderPriority = 40,
        )
        ...
        val row = makeExercise(
            id = 3, name = "Barbell Row", movementType = MovementType.COMPOUND,
            primaryGroupId = 3, orderPriority = 30,
        )

        val result = useCase(listOf(chestPress, squat, row))

        assertThat(result.map { it.name }).containsExactly(
            "Barbell Squat", "Barbell Row", "Bench Press",
        ).inOrder()
    }
    ...
    fun `core exercises always last regardless of compound or isolation status`() {
        val coreCompound = makeExercise(
            id = 1, name = "Hanging Leg Raise", movementType = MovementType.COMPOUND,
            primaryGroupId = 7, orderPriority = 70,
        )
        val armIsolation = makeExercise(
            id = 2, name = "Barbell Curl", movementType = MovementType.ISOLATION,
            primaryGroupId = 6, orderPriority = 60,
        )
        val chestCompound = makeExercise(
            id = 3, name = "Bench Press", movementType = MovementType.COMPOUND,
            primaryGroupId = 4, orderPriority = 40,
        )
        ...
        assertThat(result.map { it.name }).containsExactly(
            "Bench Press", "Barbell Curl", "Hanging Leg Raise",
        ).inOrder()
    }
    ...
    fun `within same priority, advanced exercises come before beginner`() {
        val beginner = makeExercise(
            id = 1, name = "Lat Pulldown", movementType = MovementType.COMPOUND,
            primaryGroupId = 3, orderPriority = 30, difficulty = Difficulty.BEGINNER,
        )
        val advanced = makeExercise(
            id = 2, name = "Pull-Up", movementType = MovementType.COMPOUND,
            primaryGroupId = 3, orderPriority = 30, difficulty = Difficulty.INTERMEDIATE,
        )
    ...
    fun `core isolations sorted by difficulty`() {
        val plank = makeExercise(
            id = 1, name = "Plank", movementType = MovementType.ISOLATION,
            primaryGroupId = 7, orderPriority = 70, difficulty = Difficulty.BEGINNER,
        )
        val dragonFlag = makeExercise(
            id = 2, name = "Dragon Flag", movementType = MovementType.ISOLATION,
            primaryGroupId = 7, orderPriority = 70, difficulty = Difficulty.ADVANCED,
        )
        val pallofPress = makeExercise(
            id = 3, name = "Pallof Press", movementType = MovementType.ISOLATION,
            primaryGroupId = 7, orderPriority = 70, difficulty = Difficulty.INTERMEDIATE,
        )

        val result = useCase(listOf(plank, dragonFlag, pallofPress))

        assertThat(result.map { it.name }).containsExactly(
            "Dragon Flag", "Pallof Press", "Plank",
        ).inOrder()
    }
